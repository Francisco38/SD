...
class db2 {
     ...
     */
    public db2() throws RemoteException {
    ...
    public synchronized void setUsers() {
        ...
        for (User user : this.usersInfo) {
            ...
            try {
                ...
                //open file
                File usersFile = new File("bd2\\users.txt");
                if (!usersFile.exists()) {
                ...
                System.out.println("Exception while updating users File: " + e.getMessage());
            }
        }
        needupdate("bd2\\users.txt", 1);
    }
    ...
    public synchronized void needupdate(String dir, int v) {
        try {
            ...
            System.out.println("interruptedException caught in needupdate function :" + e.getMessage());
        }
    }
    ...
    public synchronized void userUpdate() {
        try {
            ...
            System.out.println("interruptedException caught in userupdate function:" + e.getMessage());
        }
    }
    ...
    public synchronized void checkWhenNedded() {
        try {
            ...
            System.out.println("interruptedException caught checkWhenNedded function: " + e.getMessage());
        }
    }
 ...
 * @author Iago Bebiano
 */
class ucDriveServer_admin2 extends UnicastRemoteObject implements ucDrive {
    //Data base
    db2 db;
     ...
     * @throws RemoteException 
     */
    public ucDriveServer_admin2(db2 dbInput) throws RemoteException {
        db = dbInput;
    }
    ...
    public String register(User userData) throws RemoteException {
        ...
        try {
            ...
            File file = new File("bd2\\dir\\" + userData.cc[0]);
            if (!file.exists()) {
                if (file.mkdir()) {
            ...
            file = new File("bd\\dir\\" + userData.cc[0]);
            if (!file.exists()) {
                if (file.mkdir()) {
                    // System.out.println("Directory is created!");
                } else {
    ...
    public File[] listFiles(String user_id) throws RemoteException {
        File file = new File("bd2\\dir\\" + user_id);
        try {
    ...
    public long storageInfo(String user_id) throws RemoteException {
        ...
        if (user_id.equals("all")) {
            file = new File("bd2\\dir");
        } else {
            file = new File("bd2\\dir\\" + user_id);
        }
        if (file.exists() && file.isDirectory()) {
...
class ucDriveServer_client2 extends Thread {
     ...
     * @param dbInput 
     */
    public ucDriveServer_client2(Socket aClientSocket, int numero, db2 dbInput) {
        db = dbInput;
    ...
    private void changeServerDirec() {
        try {
            ...
            if (new_path.equals("--")) {
                ...
                currentServerDir = "bd2\\dir\\" + info[2];
 ...
 * @author Iago Bebiano
 */
public class ucDriveServer2 extends UnicastRemoteObject {    
    //server ports
    ...
    static int isPrimary = 0;

    /**
     * Constructor
     * @throws RemoteException 
     */
    public ucDriveServer2() throws RemoteException {
        super();
    }
     ...
     * @throws RemoteException 
     */
    private static db2 getUsers() throws RemoteException {
        db2 db = new db2();
        try {
            //opens users file
            File usersFileObj = new File("bd2\\users.txt");
            Scanner reader = new Scanner(usersFileObj);
     ...
     */
    public static void inCaseItsPrimary(db2 db) throws InterruptedException {
        //thread for client
        Thread t = new Thread() {
            public void run() {
                try {
                    // cria objecto para rmi
                    ucDriveServer_admin2 h = new ucDriveServer_admin2(db);
            ...
            public void run() {
                ...
                try ( ServerSocket listenSocket = new ServerSocket(serverPort)) {
                    ...
                    while (true) {
                        ...
                        numero++;
                        new ucDriveServer_client2(clientSocket, numero, db);
                    }
                } catch (IOException e) {
            ...
            public void run() {
                while (true) {
                    try ( DatagramSocket ds = new DatagramSocket()) {
                        ...
                        DatagramPacket dp = new DatagramPacket(dir, dir.length, ia, udpServerPort);
                        ds.send(dp);
                        BufferedInputStream bis = new BufferedInputStream(new FileInputStream(temp));
                        int length = bis.available();
                        byte[] buf = new byte[length];
                        bis.read(buf);
                        dp = new DatagramPacket(buf, buf.length, ia, udpServerPort);
                        ds.send(dp);
                        db.needUpdateDir.remove(0);
                        if (temp.equals("bd2\\users.txt")) {
                            db.userUpdate();
                        }
                    } catch (Exception e) {
                        e.printStackTrace();
     ...
     * @throws InterruptedException 
     */
    public static void secondaryWord(db2 db) throws RemoteException, InterruptedException, SocketException {
        // thread for failover
        Thread t = new Thread() {
            public void run() {
                int count = 1;
        ...
        Thread t2 = new Thread() {
            public void run() {
                while (true) {
                    try (DatagramSocket ds = new DatagramSocket(udpServerPort+1)){
                        byte[] rbuf = new byte[1024];
                        DatagramPacket rdp = new DatagramPacket(rbuf, rbuf.length);
                        ds.receive(rdp);

                        String tempdir = new String(rdp.getData(), 0, rdp.getLength());
                        System.out.println("dir2:" + tempdir);
                        ...
                        ds.receive(rdp);
                        fos.write(rdp.getData(), 0, rdp.getLength());
                        fos.flush();
                        fos.close();
                    } catch (Exception e) {
                        e.printStackTrace();
